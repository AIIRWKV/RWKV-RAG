...
template <typename F>
__global__ void kernel_forward(const int B, const int T, const int C, const int H, const int * const mask,
                               ...
                               F *__restrict__ const _y)
{
    ...
    {
        const int mask_value = mask[b*T+step];
        step++;
        __syncthreads();
        w[i] = exp(_w[t]);
        r[i] = float(_r[t]);
...
template <typename F>
__global__ void kernel_backward_111(const int B, const int T, const int C, const int H,const int * const mask,
...
template <typename F>
__global__ void kernel_backward_222(const int B, const int T, const int C, const int H,const int * const mask,
    const F *__restrict__ const _r, const F *__restrict__ const _k, const F *__restrict__ const _v, const float *__restrict__ _w, const F *__restrict__ _u, const F *__restrict__ const _gy,
...
template <typename F>
__global__ void kernel_backward_333(const int B, const int T, const int C, const int H,const int * const mask,
    const F *__restrict__ const _r, const F *__restrict__ const _k, const F *__restrict__ const _v, const float *__restrict__ _w, const F *__restrict__ _u, const F *__restrict__ const _gy,
    F *__restrict__ const _gw)
{
    const int b = blockIdx.x / H;
...
void cuda_forward(int B, int T, int C, int H, const int * const mask, bf16 *r, bf16 *k, bf16 *v, float *w, bf16 *u, bf16 *y)
{
    assert(H*_N_ == C);
    assert(_N_%4 == 0);
    kernel_forward<<<dim3(B * H), dim3(_N_)>>>(B, T, C, H,mask, r, k, v, w, u, y);
}
...
void cuda_backward(int B, int T, int C, int H, const int * const mask, bf16 *r, bf16 *k, bf16 *v, float *w, bf16 *u, bf16 *gy, bf16 *gr, bf16 *gk, bf16 *gv, bf16 *gw, bf16 *gu)
{
    assert(H*_N_ == C);
    assert(_N_%4 == 0);
    kernel_backward_111<<<dim3(B * H), dim3(_N_)>>>(B, T, C, H,mask, r, k, v, w, u, gy, gr, gk, gv, gu);
    kernel_backward_222<<<dim3(B * H), dim3(_N_)>>>(B, T, C, H,mask, r, k, v, w, u, gy, gw);
    kernel_backward_333<<<dim3(B * H), dim3(_N_)>>>(B, T, C, H,mask, r, k, v, w, u, gy, gw);
}